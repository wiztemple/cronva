...
export class TvFetcher {
  async syncAll() {
    const events = tvEvents as TvEvent[]

    const bySlug = new Map<string, TvEvent[]>()
    ...
    for (const [slug, calEvents] of bySlug.entries()) {
      ...
      if (!calendar) {
        console.warn(`[TvFetcher] Calendar not found: ${slug}`)
        continue
      }
      ...
      console.log(`[TvFetcher] ${slug}: ${upserted} events upserted`)
    }
  }
}
